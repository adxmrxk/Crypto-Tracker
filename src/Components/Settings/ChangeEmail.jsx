...
const ChangeEmail = ({ clickChangeEmail, setClickChangeEmail }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    e.preventDefault();
    const newEmailValue = e.target.newEmail.value;
    const confirmNewEmailValue = e.target.confirmNewEmail.value;
    ...
    const res = await axios.patch(`http://localhost:5000/api/settings/changeEmail/${user._id}`, {email: newEmailValue});
  ...
  const handleClose = () => setClickChangeEmail(!clickChangeEmail);
      ...
      <div className="relative bg-gradient-to-br from-slate-600 via-slate-700 to-slate-800 w-full max-w-md rounded-xl p-6 shadow-2xl ring-1 ring-gray-500/40 transition-transform duration-200 scale-100 hover:scale-[1.01]">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <input
            ...
            className="outline-none bg-slate-500/50 text-blue-100 placeholder-gray-300 rounded-md px-3 py-2 focus:ring-1 focus:ring-blue-500 focus:bg-slate-600 transition"
            required
          />
