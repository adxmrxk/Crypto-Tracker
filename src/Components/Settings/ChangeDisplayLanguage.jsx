...
const ChangedisplayLanguage = ({ clickChangedisplayLanguage, setClickChangedisplayLanguage }) => {
  ...
  const handleClose = () => setClickChangedisplayLanguage(!clickChangedisplayLanguage);
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 px-4 backdrop-blur-sm bg-black/40">
      <div className="relative bg-gradient-to-br from-slate-600 via-slate-700 to-slate-800 w-full max-w-md rounded-xl p-6 shadow-2xl ring-1 ring-gray-500/40 transition-transform duration-200 scale-100 hover:scale-[1.01]">
        ...
        <div className="flex items-center justify-between mb-4">
          <h1 className="font-semibold text-xl text-gray-100 tracking-wide">Change your displayLanguage</h1>
          <X size={20} className="cursor-pointer text-gray-300 hover:text-white transition" onClick={handleClose}/>
        </div>

        <p className="text-gray-300/90 mb-6 -mt-3 text-sm text-left w-[300px]">Please enter your new display language.</p>

        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          <select name = 'newdisplayLanguage' defaultValue='' className="outline-none appearance-none bg-slate-500/50 text-gray-100 placeholder-gray-300 rounded-md px-3 py-2 focus:ring-1 focus:ring-blue-500 focus:bg-slate-600 transition" required placeholder="New displayLanguage">

             <option value="" disabled hidden>Select a display language</option>
             {displayLanguages.map((displayLanguage, index) => (
               <option key = {index} type = "text">{displayLanguage}</option>
              ))}
          </select>
