...
const SingleCryptoCard = ({ searchedCoin, submitedSearch, coinsList }) => {
  ...
  const userCoin = coinsList?.filter(
    (coin) => coin.id === searchedCoin?.toLowerCase()
  );
  const coinData = userCoin?.[0];
  console.log("User Coin", userCoin);
  console.log("userCoin name", coinData?.name);
  ...
  // Show message if coin not found in the API data
  if (!coinData) {
    return (
      <div className="grid grid-cols-3 gap-4 p-3 w-[1500px]">
        <div className="bg-gradient-to-r from-gray-600 via-gray-700 to-gray-700 ring ring-amber-400/50 w-auto rounded-xs p-6">
          <h1 className="text-xl text-gray-100 font-semibold">
            Coin Not Available
          </h1>
          <p className="text-gray-300 mt-2">
            "{searchedCoin}" is not currently available in the market data. This
            may be a lower market cap coin not in the top listings.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="grid grid-cols-3 gap-4 p-3 w-[1500px]">
      {searchedCoin !== "Bitcoin" && coinData?.name === "Bitcoin" ? null : (
        <div className="bg-gradient-to-r from-gray-600 via-gray-700 to-gray-700 ring ring-amber-400/50 w-auto rounded-xs cursor-pointer hover:scale-102 transition-all duration-250 ease-in-out">
          <div className="flex flex-row gap-2 items-center ml-3 mt-3">
            <img
              src={coinData?.image}
              alt={coinData?.name}
              className="w-13 h-13 rounded-full mb-30"
            ></img>
            <div className="flex flex-col">
              <h1 className="text-2xl text-gray-100 font-semibold text-left">
                {coinData?.name}
              </h1>
              ...
              <div className="mt-5">
                <div className="w-fit ml-[320px] -mb-[67px] flex flex-col items-center">
                  <form onSubmit={(event) => handleSubmit(event, coinData)}>
                    <button
                      type="submit"
                ...
                <div className="flex flex-row items-center mb-5 mt-10 -translate-x-9">
                  ...
                  <h1
                    className={`text-left text-lg font-semibold text-gray-100 ${
                      coinData?.name ? "bg-blue-500/20" : ""
                    } rounded-xs px-1 ml-2`}
                  >
                    $
                    {Number(
                  ...
                  <h1 className="text-md font-normal text-gray-300 ml-3">
                    24h:{" "}
                  </h1>
                  <h1
