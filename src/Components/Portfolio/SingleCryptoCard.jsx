    ...
    <div className='grid grid-cols-3 gap-4 p-3 w-[1500px]'>
      {searchedCoin !== 'Bitcoin' && data?.[0]?.name === 'Bitcoin' ? null : <div className='bg-gradient-to-r from-gray-600 via-gray-700 to-gray-700 ring ring-sky-300 w-auto rounded-xs cursor-pointer hover:scale-102 transition-all duration-250 ease-in-out'>
                        <div className='flex flex-row gap-2 items-center ml-3 mt-3'>
                            ...
                            <img src={data?.[0]?.image} alt={data?.[0]?.name} className='w-13 h-13 rounded-full mb-30'></img>
                            <div className='flex flex-col'>
                                <h1 className='text-2xl text-gray-100 font-semibold text-left'>{data?.[0]?.name}</h1>
                                <h1 className='text-lg text-gray-300 font-normal text-left'>{data?.[0]?.symbol.toUpperCase()}</h1>
                                <div className='mt-5'>
                                    ...
                                    <div className='flex flex-row items-center mb-5 mt-10 -translate-x-9'>
                                        ...
                                        <h1 className='text-left text-lg font-semibold text-gray-100 bg-blue-500/20 rounded-xs px-1 ml-2'>${Number(data?.[0]?.current_price.toFixed(2)).toLocaleString()}</h1>
