...
const CryptoCurrencyCard = ({searchedCoin, submitedSearch, setSubmitedSearch}) => {
    ...
    const handleSubmit = async (event, element) => {
        ...
        const response = await axios.patch(`http://localhost:5000/api/addWatchList/${user._id}`, { $addToSet: { watchList: { coin: element.id, amount: value, ticker: String(element.symbol), image: element.image} } } )
